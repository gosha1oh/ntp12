...
int main(int argc, char *argv[ ])
{
    ...
    if(argc < 7){
        ...
        fprintf(stdout, "Usage: %s <ip range start (192.0.0.0)> <ip range end (198.255.255.255)> <outfile> <threads> <scan delay in ms> <min. response>\n", argv[0]);
        exit(-1);
    }
    fd = fopen(argv[3], "a");
